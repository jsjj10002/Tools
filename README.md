...
```
main                    # 🌟 프로덕션 브랜치 (안정 릴리즈)
├── develop            # 🔄 개발 통합 브랜치 (다음 릴리즈 통합)
└── feature/           # 🚀 기능별 브랜치
    ├── image-tools      🔄 (이미지 도구 개발 중)
    └── video-tools      📝 (영상 도구 예정)
```

### 브랜치 관리 정책

프로젝트는 **Git Flow** 기반의 브랜치 전략을 따릅니다:

- **`main`**: 프로덕션 배포용 안정 브랜치 (보호됨)
...
자세한 브랜치 관리 정책은 [브랜치 관리 정책 문서](./docs/브랜치_관리_정책.md)를 참고하세요.

### 현재 브랜치 상태
- **main**: 프로덕션 배포 준비 완료 (안정)
- **develop**: 개발 통합 브랜치 (신규 기능 개발 중)
